...
class PulseBlaster(PseudoclockDevice):
    ...
    def __init__(self, name, loop_number, extra_flags, extra_inst, extra_inst_data, extra_length, inst_location, additional_inst, loop_start, trigger_device=None, trigger_connection=None, board_number=0, firmware = '',
    ...
    def convert_to_pb_inst(self, dig_outputs, dds_outputs, freqs, amps, phases):
        ...
        for k, instruction in enumerate(self.pseudoclock.clock):
            #raise LabscriptError(self.pseudoclock.clock)
            if instruction == 'WAIT':
...
class PulseblasterWorker(Worker):
    ...
    def transition_to_buffered(self,device_name,h5file,initial_values,fresh):
        ...
        with h5py.File(h5file,'r') as hdf5_file:
            ...
            self.logger.info("THIS" + str(pulse_program))
            #raise LabscriptError(PULSE_PROGRAM)
            pb_start_programming(PULSE_PROGRAM)
