...
class PulseBlaster_No_DDS(PulseBlaster):
    ...
    def write_pb_inst_to_h5(self, pb_inst, hdf5_file):
        ...
        pb_dtype= [('flags',np.int32), ('inst',np.int32), ('inst_data',np.int32), ('length',np.float64), ('loop_number',np.int32)]
        pb_inst_table = np.empty(len(pb_inst)+1,dtype = pb_dtype)
        for i,inst in enumerate(pb_inst):
            ...
            pb_inst_table[i] = (flagint, instructionint, dataint, delaydouble, loops)
            ...
            if fresh or (self.smart_cache['initial_values'] != initial_values) or \
                ...
                (self.smart_cache['pulse_program'] != pulse_program).any():
                    ...
                    for args in pulse_program:
                        pb_inst_pbonly(args[0], args[1], args[2],5*args[3])
